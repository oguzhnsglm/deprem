...
import { signInWithPassword } from '../logic/authStore';
...
const SignInScreen = ({ navigation }) => {
  ...
  const submit = async () => {
    ...
    if (!password) {
      setError(t('signInPasswordRequired'));
      return;
    }
    setLoading(true);
    try {
      ...
      setError(err?.message || t('signInGenericError'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.85}
          >
            {loading ? <ActivityIndicator color="#0a0f1e" /> : <Text style={styles.submitText}>{t('signInSubmit')}</Text>}
          </TouchableOpacity>

          <TouchableOpacity onPress={() => navigation.navigate('SignUp')} activeOpacity={0.7}>
            <Text style={styles.altLink}>{t('signInNoAccount')}</Text>
          </TouchableOpacity>
        </ScrollView>
